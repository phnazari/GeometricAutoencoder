...
class LogTrainingLoss(Callback):
    ...
    def on_epoch_end(self, epoch, **kwargs):
        ...
        # latent_activations = kwargs["latent_activations"]
        # inputs = inputs.view(latent_activations.shape[0], -1)
        # k_min, k_max, k_step = \
        ...
        # measures = evaluator.calc_metrics(evaluation["metrics"], data, latent, ks)
...
class LogDatasetLoss(Callback):
    ...
    def on_epoch_end(self, model, epoch, geom_error, **kwargs):
        ...
        # save geometric losses to file
        if self.with_geom_loss and self.save_path is not None:
            self.geom_losses.append(geom_error)
            geom_save_path = os.path.join(self.save_path, 'geometric_loss.pth')
            torch.save(torch.tensor(self.geom_losses), geom_save_path)
