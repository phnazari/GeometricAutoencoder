...
def train(val_size, evaluation, _run, _log, _seed, _rnd):
    ...
    if type(model).__name__ == "ParametricUMAP":
        model.save(os.path.join(rundir, 'model'))

    if rundir and evaluation['save_model']:
        # Save model state (and entire model)
        with open(os.path.join(rundir, 'model.pkl'), 'wb') as f:
            pickle.dump(model, f)
    ...
    if evaluation['active']:
        ...
        if rundir and evaluation['save_latents']:
            ...
            df['labels'] = labels
            df.to_csv(os.path.join(rundir, 'test_latents.csv'), index=False)
            np.savez(
            ...
            df_test.to_csv(os.path.join(rundir, 'test_inputs.csv'), index=False)
        ...
        if latent.shape[1] == 2 and rundir:
            ...
            visualize_latents(
                latent, labels,
                save_file=os.path.join(rundir, 'latent_visualization.png')
            )
