...
class UMAPAE(AutoencoderModel):
    ...
    def __init__(self, input_dims=(1, 28, 28), **kwargs):
        ...
        self.encoder = tf.keras.Sequential(
            [
                tf.keras.layers.InputLayer(input_shape=n_input_dims),
                tf.keras.layers.Activation("elu"),
                tf.keras.layers.Flatten(),
                tf.keras.layers.Dense(units=100, activation="elu"),
                tf.keras.layers.Dense(units=100, activation="elu"),
                tf.keras.layers.Dense(units=100, activation="elu"),
                tf.keras.layers.Dense(units=self.latent_dim, name="z"),
            ]
        )
        ...
        self.decoder = tf.keras.Sequential(
            [
                ...
                tf.keras.layers.Dense(
                    units=n_input_dims, name="recon", activation=None
                ),
    ...
    def embedd(self, x_train):
        ...
        embedder = ParametricUMAP(encoder=self.encoder,
...
class ELUUMAPAutoEncoder(AutoencoderModel):
    """100-100-100-2-100-100-100."""
