...
def plot_latent_space(model,
    ...
    if with_legend:
        if dataset_name == "Earth":
            ...
            string_labels = ["T-shirt/top", "Trouser", "Pullover", "Dress", "Coat", "Sandal", "Shirt", "Sneaker", "Bag",
                             "Ankle boot"]

            legend = ax.legend(handles, string_labels, loc="center left", bbox_to_anchor=(1.5, 0.5), ncol=2,
                               fontsize=22,
                               markerscale=2)
        else:
            chartBox = ax.get_position()
    ...
    if output_path is not None:
        ...
        # plt.savefig(output_path, format="png", pad_inches=0, dpi=400)
