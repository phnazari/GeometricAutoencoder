...
class Multi_Evaluation:
    ...
    def calc_metrics(self, metrics, data, latent, ks):
        '''
        Performs evaluation for nonlinear dimensionality reduction.

        - metrics: name of metric to be used
        ...
        # classify used metrics
        used_k_dependant_measures_keys = list(set(calc.measures.k_dependent_measures.keys()) & set(metrics))
        used_k_independant_measures_keys = list(set(calc.measures.k_independent_measures.keys()) & set(metrics))

        # get the corresponding functions
        used_k_dependant_measures = {key: calc.measures.k_dependent_measures[key] for key in
                                     used_k_dependant_measures_keys}
        used_k_independant_measures = {key: calc.measures.k_independent_measures[key] for key in
                                       used_k_independant_measures_keys}

        k_dependant_measures = {
            key: np.array([fn(calc, k) for k in ks])
            for key, fn in used_k_dependant_measures.items()
        }

        k_independant_measures = {
            key: fn(calc) for key, fn in used_k_independant_measures.items()
        }

        measures = {**k_independant_measures, **k_dependant_measures}
